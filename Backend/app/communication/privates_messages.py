...
def send_message(message: Message):
    ...
    try:
        ...
        if not get_sender:
            raise HTTPException(status_code=501, detail="Sender does not exist.")
        cursor.execute("SELECT id FROM users where email = %s", (reciver_email,))
        get_reciver = cursor.fetchone()
        if not get_reciver:
            raise HTTPException(status_code=501, detail="The email you tried to send this message does not exist.")
        if (get_sender == get_reciver):
            raise HTTPException(status_code=501, detail="Sender and Reciver of the massages cannot be the same.")
...
def read_message(reader: Read_conversation_from):
    ...
    try:
        cursor.execute("SELECT id, name FROM users where email = %s", (reader_email,))
        get_reader = cursor.fetchone()
        if not get_reader:
            raise HTTPException(status_code=501, detail="Reader does not exist.")
        cursor.execute("SELECT id, name FROM users where email = %s", (readed_email,))
        get_readed = cursor.fetchone()
        if not get_readed:
            raise HTTPException(status_code=501, detail="The email you tried to read your conversation with does not exist.")
        if (get_reader == get_readed):
            raise HTTPException(status_code=501, detail="You cannot try to read a conversation with yourself.")
        reader_id: int = get_reader['id']
        readed_id: int = get_readed['id']
        reader_name: str = get_reader['name']
        readed_name: str = get_readed['name']
        cursor.execute("SELECT id FROM conversations where user1_id = %s AND user2_id = %s OR user1_id = %s AND user2_id = %s", (reader_id, readed_id, readed_id, reader_id,))
        get_conv = cursor.fetchone()
        if get_conv:
            conv_id: int = get_conv['id']
            cursor.execute("SELECT sender_id, content FROM messages where conversation_id = %s", (conv_id,))
            ...
            raise HTTPException(status_code=501, detail="No conversation with this email yet.")
    finally:
        cursor.close()
        connection.close()