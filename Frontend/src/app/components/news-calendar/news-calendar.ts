...
@Component({
  selector: 'app-news-calendar',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './news-calendar.html',
  styleUrls: ['./news-calendar.css']
})
export class NewsCalendar implements OnChanges {
  @Input() news: News[] = [];
  ...
  weeks: DayCell[][] = [];
  monthLabel = '';
  private newsByDay = new Map<string, number>(); // YYYY-MM-DD -> count
  ...
  private indexNews() {
    this.newsByDay.clear();
    for (const n of this.news) {
      const d = n.news_date?.slice(0, 10);
      if (!d) continue;
      this.newsByDay.set(d, (this.newsByDay.get(d) ?? 0) + 1);
    }
  }
  ...
  private buildCalendar() {
    ...
    while (cursor <= endDay) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const count = this.newsByDay.get(iso) ?? 0;
        week.push({
